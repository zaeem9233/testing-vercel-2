...
// Lazy load fast-glob only when needed (not during build)
let fg: typeof import('fast-glob').default | null = null;
async function getFastGlob() {
  if (!fg && typeof process !== 'undefined' && process.env.NODE_ENV !== 'production') {
    try {
...
export async function loader({ params }: Route.LoaderArgs) {
  // Get the missing path, default to empty string if undefined
  const missingPath = String(params?.['*'] || '');
  
  // During build/prerender, always return minimal safe data
  // The catch-all route doesn't need to be prerendered anyway
  if (typeof process !== 'undefined' && process.env.NODE_ENV === 'production') {
    return {
      path: missingPath ? `/${missingPath}` : '/',
      pages: [],
    };
  }
  
  // In development, try to get page files
  let pages: Array<{ url: string; path: string }> = [];
  
  try {
    const fastGlob = await getFastGlob();
    if (fastGlob) {
      const matches = await fastGlob('src/**/page.{js,jsx,ts,tsx}');
      
      // Ensure all data is serializable and valid
      pages = matches
        ...
        .map((match) => {
          try {
            ...
            return { 
              url: String(url || '/'), 
              path: String(displayPath || 'Homepage') 
            };
          } catch {
            return null;
          }
        })
        .filter((page): page is { url: string; path: string } => 
          page !== null && typeof page.url === 'string' && typeof page.path === 'string'
        );
    }
  } catch (error) {
    // If anything fails, just use empty array
    // Don't log in production to avoid build errors
    if (typeof process === 'undefined' || process.env.NODE_ENV !== 'production') {
      console.warn('Error in not-found loader:', error);
    }
  }
  
  // Always return a safe, serializable response
  return {
    path: missingPath ? `/${missingPath}` : '/',
    pages,
  };
